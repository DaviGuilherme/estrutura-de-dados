...
//            }
    ...
    public void adicionaNoMeio(int elementoNovo) {
        if(nroElem == vetor.length || nroElem == 0){
            throw new IllegalStateException();
        }else {
            for(int i = nroElem/2; i < nroElem;i++){
                vetor[i] = vetor[i+1];
            }
            vetor[nroElem/2] = elementoNovo;
        }
    }
    ...
    public void listaParaMatriz(int[][] m) {
        if(nroElem == m.length){
            for(int linha = 0; linha < m.length;linha++){
                for(int coluna = 0; coluna < m[linha].length;coluna++){
                    m[linha][coluna] = vetor[linha];
                }
            }
        }else {
